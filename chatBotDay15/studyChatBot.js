...
function pushAnswer() {
  orderSet.push(`${question}`);
  answerSet.push({ order: `${question}`, answer: `${saveAnswer}` });
  answerBox.innerHTML = '말을 배웠다냥';
  console.log(orderSet);
  key = 0;
}
function pushAnswerMain() {
  const orderData = getInputData();
  const orderFilter = orderSet.filter((obj) => obj === orderData);
  console.log(orderFilter);
  if (key == 1) {
    if (orderData == '응') {
      answerBox.innerHTML = '가르칠 대답을 입력하세요';
      key = 2;
    } else {
  ...
  if (key == 2) {
    saveAnswer = orderData;
    window.addEventListener('keydown', (e) => {
      if (e.keyCode == 13) pushAnswer();
    });
  }
  if (orderFilter[0] === orderData) {
    ...
    defaultBack();
    answerBox.innerHTML = '말 가르쳐주라냥(응/아니)';
    question = orderData;
    key = 1;
  }
}

orderBtn.addEventListener('click', pushAnswerMain);
